import streamlit as st
import pandas as pd
import numpy as np
import plotly.graph_objects as go
from plotly.subplots import make_subplots

# --- 1. MÜHENDİSLİK MOTORU (HESAPLAMALAR) ---

def get_bearing_capacity_factors(phi_deg):
    """Taşıma gücü katsayılarını (Nc, Nq, Ny) hesaplar"""
    phi_rad = np.radians(phi_deg)
    if phi_deg == 0:
        return 5.14, 1.0, 0.0
    Nq = np.tan(np.radians(45 + phi_deg/2))**2 * np.exp(np.pi * np.tan(phi_rad))
    Nc = (Nq - 1) / np.tan(phi_rad)
    Ny = 2 * (Nq + 1) * np.tan(phi_rad) # Vesic/Hansen yaklaşımı
    return round(Nc, 2), round(Nq, 2), round(Ny, 2)

def process_geotech_data(df, B, L, Df, dw, Mw, a_max):
    # --- Veri Hazırlık ---
    df.columns = df.columns.str.strip().str.upper()
    for col in ['DEPTH', 'SPT_N', 'UNIT_WEIGHT']:
        if col in df.columns:
            df[col] = pd.to_numeric(df[col].astype(str).str.replace(',', '.'), errors='coerce')
    df = df.dropna(subset=['DEPTH', 'SPT_N']).reset_index(drop=True)

    # --- SPT Düzeltmeleri ---
    def get_cr(d):
        if d < 3: return 0.75
        elif 3 <= d < 4: return 0.80
        elif 4 <= d < 6: return 0.85
        elif 6 <= d < 10: return 0.95
        else: return 1.0

    gamma_w = 9.81
    df['Cr'] = df['DEPTH'].apply(get_cr)
    df['N60'] = df['SPT_N'] * df['Cr']
    
    # Gerilme Analizi
    df['Sigma_V'] = df['DEPTH'] * df['UNIT_WEIGHT']
    df['u'] = df.apply(lambda r: (r['DEPTH'] - dw) * gamma_w if r['DEPTH'] > dw else 0, axis=1)
    df['Sigma_V_Eff'] = df['Sigma_V'] - df['u']
    
    # C_N ve N1_60
    df['C_N'] = np.sqrt(100 / df['Sigma_V_Eff'].replace(0, 10)).clip(upper=2.0)
    df['N1_60'] = df['N60'] * df['C_N']

    # --- Parametre Kestirimi ---
    # Phi (Kumlar için) ve Cu (Killer için - Yaklaşık)
    df['phi'] = (27.1 + 0.3 * df['N1_60'] - 0.00054 * (df['N1_60']**2)).round(1)
    df['cu'] = (df['N60'] * 6.25).round(1) # kPa (Yaklaşık drenajsız kayma mukavemeti)

    # --- Sıvılaşma Analizi (Idriss & Boulanger) ---
    msf = 6.9 * np.exp(-Mw/4) - 0.058
    rd = 1.0 - 0.00765 * df['DEPTH']
    df['CSR'] = 0.65 * (a_max) * (df['Sigma_V']/df['Sigma_V_Eff']) * rd
    df['CRR'] = np.exp((df['N1_60']/14.1) + ((df['N1_60']/126)**2) - 2.8) * msf
    df['FS_Liq'] = (df['CRR'] / df['CSR']).round(2)

    # --- TAŞIMA GÜCÜ ANALİZİ (Her Derinlikte Potansiyel Kapasite) ---
    q_results = []
    for i, row in df.iterrows():
        Nc, Nq, Ny = get_bearing_capacity_factors(row['phi'])
        # Şekil Faktörleri (Meyerhof)
        sc = 1 + 0.2 * (B/L)
        sq = 1 + 0.1 * (B/L)
        sy = 1 + 0.1 * (B/L)
        
        # Nihai Taşıma Gücü (Terzaghi/General)
        # Sadece kumlu zemin varsayımıyla (c=0) basitleştirilmiş:
        q_ult = (row['Sigma_V_Eff'] * Nq * sq) + (0.5 * row['UNIT_WEIGHT'] * B * Ny * sy)
        q_all = q_ult / 3.0 # Güvenlik Katsayısı = 3
        q_results.append(round(q_all, 1))
    
    df['Q_Allowable_kPa'] = q_results
    return df

# --- 2. GÖRSELLEŞTİRME ---

def draw_plots(df):
    fig = make_subplots(rows=1, cols=3, shared_yaxes=True, 
                        subplot_titles=("N1(60) Profili", "Sıvılaşma FS", "Emniyetli Taşıma (kPa)"))

    fig.add_trace(go.Scatter(x=df['N1_60'], y=df['DEPTH'], name='N1(60)'), row=1, col=1)
    fig.add_trace(go.Scatter(x=df['FS_Liq'], y=df['DEPTH'], name='FS', line=dict(color='red')), row=1, col=2)
    fig.add_trace(go.Scatter(x=df['Q_Allowable_kPa'], y=df['DEPTH'], name='q_all', line=dict(color='green')), row=1, col=3)
    
    fig.add_vline(x=1.0, line_dash="dash", line_color="black", row=1, col=2)
    fig.update_yaxes(autorange="reversed", title_text="Derinlik (m)")
    fig.update_layout(height=800, template="plotly_dark", title_text="GeoTech-AI 200m Derin Analiz Paneli")
    return fig

# --- 3. STREAMLIT ARAYÜZÜ ---

st.set_page_config(page_title="GeoTech-AI Pro", layout="wide")
st.title("🏗️ GeoTech-AI: Profesyonel Geoteknik Analiz")

with st.sidebar:
    st.header("🛠️ Saha & Tasarım")
    B = st.number_input("Temel Genişliği (B) [m]", 1.0)
    L = st.number_input("Temel Boyu (L) [m]", 1.0)
    Df = st.number_input("Temel Derinliği (Df) [m]", 1.5)
    dw = st.number_input("Su Seviyesi (dw) [m]", 3.0)
    Mw = st.number_input("Magnitüd (Mw)", 7.5)
    amax = st.number_input("İvme (amax) [g]", 0.4)
    st.markdown("---")
    uploaded_file = st.file_uploader("200m Analiz Dosyası Yükle", type=['xlsx', 'csv'])

if uploaded_file:
    try:
        df_in = pd.read_csv(uploaded_file, decimal=",") if uploaded_file.name.endswith('.csv') else pd.read_excel(uploaded_file)
        df_res = process_geotech_data(df_in, B, L, Df, dw, Mw, amax)
        
        t1, t2 = st.tabs(["📝 Veri Tablosu", "📊 Analiz Grafikleri"])
        with t1:
            st.dataframe(df_res.style.format(precision=2), use_container_width=True)
        with t2:
            st.plotly_chart(draw_plots(df_res), use_container_width=True)
            
    except Exception as e:
        st.error(f"Hata: {e}")
else:
    st.info("👈 Analiz için lütfen Excel dosyanızı yükleyin.")
